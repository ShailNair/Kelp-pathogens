...
database_dir <- "~/database/"  # databases
```
...
33F and 806R primers were used
```{r}
FWD <- "CCTAYGGGRBGCASCAG"  ## CHANGE ME to your forward primer sequence
REV <- "GGACTACNNGGGTATCTAAT"  ## CHANGE ME to your reverse primer sequence
```
...
#Be careful while setting trunclength, Consider your sequenced amplicon length and check the quality profiles from above graph 
```{r}
out <- filterAndTrim(cut_R1, filt_R1, cut_R2, filt_R2,
...
```{r}
db_file <- paste0(database_dir, "GTDB_bac120_arc53_ssu_r207_Genus.fa.gz")
taxa <- assignTaxonomy(seqtab.nochim, refFasta = db_file, multithread=TRUE)
``` 

# 4.7.1 Assign species and save Rdata
```{r}
taxa <- addSpecies(taxa, "~/database/GTDB_bac120_arc53_ssu_r207_Species.fa.gz")
``` 
...
write(asv_fasta, str_c(dada2_dir, "kelp.16s.ASVs.fa"))
```
...
write.table(asv_tab, "~/dada2/kelp.ASVs_counts.tsv", sep="\t", quote=F, col.names=NA)
```
...
write.table(asv_tax, "~/dada2/kelp.ASVs_taxonomy.gtdb.tsv", sep = "\t", quote=F, col.names=NA)
```

# To merge asv abundance and taxonomy into one file
```{r}
OTU_TAX_table <- merge(asv_tab, asv_tax, by=0)
write.table(OTU_TAX_table, "~/dada2/kelp.OTU_TAX_table.gtdb.tsv", sep = "\t", quote=F, col.names=NA)
```

## 4.9 save R data

```{r}
save.image(file = "~/dada2/kelp_16s_analysis.rds.RData")
``` 